...
if($clientRoomEncodeData){
    ...
    $payer_user_email = find_lead_email_in_UTIP($payer_user_id,'kutiy6tryL4fNx8','#','54891247','admin@simpletrading.biz','nKC05dWLD3dHGzd');
}
...
if($donation_amount){
    add_data_to_db($global_main_db_name_api,$payer_account_id,$payer_user_id,$payer_user_email,$donation_amount, $direction, $global_test_message . $brand,'no link', 'Generate link in process', $all_tragers_room_data);
    ...
    if($parserClientDataResponse[1] == 'Success'){ 
        update_data_in_db($global_main_db_name_api,$id_operation, 'Success: Link was generated', $parserClientDataResponse[0]);
        ...
        update_data_in_db($global_main_db_name_api,$id_operation, 'Error: Error generate link','no link');
